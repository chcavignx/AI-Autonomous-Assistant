...
def test_path_config_properties():
    pc = config.PathConfig()
...
def test_tts_config_defaults():
    ...
    assert tts.model_name == "en_US-hfc_female-medium.onnx"
...
def test_tts_config_full_model_path_default():
    tts = config.TTSConfig()
    expected = config.ROOT_DIR / ".cache" / "audio" / "models" / "piper" / "en_US-hfc_female-medium.onnx"
    assert tts.full_model_path == expected


def test_tts_config_full_model_path_with_path():
...
def test_platform_config_is_raspberry_pi(monkeypatch):
    platform = config.PlatformConfig()
...
def test_platform_config_cpu_limit(monkeypatch):
    platform = config.PlatformConfig()
...
def test_asr_config_download_path_with_engine_in_path():
    asr = config.ASRConfig(download_root="faster-whisper")
    expected = config.ROOT_DIR / "faster-whisper"
    assert asr.download_path == expected


def test_tts_config_full_model_path_file():
    tts = config.TTSConfig(model_path="custom/model.onnx")
    expected = config.ROOT_DIR / "custom" / "model.onnx"
    assert tts.full_model_path == expected


def test_wake_config_download_path_with_suffix():
...
def test_load_config_with_path(monkeypatch):
    # Mock yaml.safe_load to return config dict
    mock_config_dict = {"asr": {"model_size": "large"}}
    monkeypatch.setattr(config.yaml, "safe_load", lambda f: mock_config_dict)
    # Mock exists to True
...
def test_load_config_invalid_path(monkeypatch):
...
def test_load_config_yaml_none(monkeypatch):
    monkeypatch.setattr(config.yaml, "safe_load", lambda f: None)
    monkeypatch.setattr(pathlib.Path, "exists", lambda self: True)
    from io import StringIO
