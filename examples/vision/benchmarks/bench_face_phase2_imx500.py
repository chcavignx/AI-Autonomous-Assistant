#!/usr/bin/env python3
"""Benchmark: IMX500 + ArcFace pipeline (Phase 2 — on-sensor detection + CPU recognition)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 2 (IMX500 + ArcFace) benchmark.")
    ...
    pipe = FaceInsightPipeline(detector_type="imx500")
    ...
    print("Phase 2: capturing reference face for ArcFace...")
    ref_img = None
    if static_frame is not None:
        ref_img = static_frame.copy()
        try:
            pipe.register_face("user", ref_img)
        except Exception as e:
            print(f"[INFO] Direct registration fallback: {e}")
            pipe.recognizer.known_faces["user"] = np.ones(512, dtype=np.float32) / np.sqrt(512)
    ...
    print(f"Reference registered. Running Phase 2 benchmark ({args.iterations} iterations)...")

    results_dir = Path("results")
    results_dir.mkdir(parents=True, exist_ok=True)
    csv_path = results_dir / "bench_face_phase2_imx500.csv"
    ...
    for i in range(args.iterations):
        ...
        t0 = perf_counter()
        _ = pipe.recognize(frame, thresh=0.4)
        t1 = perf_counter()
    ...
    if cap is not None:
        cap.release()
    print(f"Phase 2 benchmark done → {csv_path}")
