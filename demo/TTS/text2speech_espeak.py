...
def setup_output_filename(base_path, base_name, suffix, counter=1) -> str:
    ...
    while os.path.exists(output_file):
        output_file = os.path.join(base_path, f"{file_name}_{suffix}_{counter}{file_extension}")
        counter += 1
...
for voice in voices:
    ...
    if (voice.languages == 'en' and 'GB') and (voice.gender == VoiceGenderMale) in voice.id:
        voice_id = voice 
...
engine.save_to_file("Hello, How are you. My name is JARVIS.", 
             setup_output_filename(DATA_DIR, TEST_FILE_NAME, f'{voices[voice_id].name}_{voices[voice_id].languages}'))

# Clean up and release resources
engine.stop()
print("Speech synthesis complete.")
print(f"Audio file saved to: {setup_output_filename(DATA_DIR, TEST_FILE_NAME, f'{voices[voice_id].name}_{voices[voice_id].languages}')}")
