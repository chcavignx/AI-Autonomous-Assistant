#!/usr/bin/env python3
"""
This script demonstrates how to run automatic speech recognition (ASR) using Hugging Face's Transformers and Datasets libraries,
with optimizations for low-resource devices such as the Raspberry Pi. It loads a Whisper model from local cache, processes an audio file,
and outputs the transcription. The script includes system resource monitoring, device-specific optimizations, and error handling.
...
if detect_raspberry_pi_model():
    ...
    #Sdataset_name = "distil-whisper/librispeech_long"
else:
...
model = AutoModelForSpeechSeq2Seq.from_pretrained(
    ...
    low_cpu_mem_usage = True if detect_raspberry_pi_model() else False,     # Critical for Pi
...
pipe = pipeline(
    ...
    device = -1,
    processor = processor,
    model_kwargs = {"low_cpu_mem_usage": True if detect_raspberry_pi_model() else False}
)
