...
import pyaudio
import subprocess
# from sysutils import limit_cpu_for_multiprocessing, detect_raspberry_pi_model
...
class AgentConfig:
    ...
    cpu_cores: Optional[int] = 4  # # Limit to 2 cores for Raspberry Pi, None to use all available cores
    ...
    faster_whisper_download_root: str = os.path.join(os.path.expanduser("~"), ".cache/huggingface")
    vosk_model_path: str = default_vosk_model_path
    
    # TTS settings
    piper_model_path: str = os.path.join(DATA_DIR, "jarvis-medium.onnx") #"en_US-amy-medium.onnx"
    ...
    tts_to_cli: bool = False  # If True, use Piper CLI for TTS synthesis else use SoundDevice for TTS playback with Python API
    # VAD settings
...
class AudioUtils:
    ...
    @staticmethod
    def play_audio_stream(audio_stream: np.ndarray, sample_rate: int, channels: int=1, dtype: str='int16') -> None:
        """Play audio stream using SoundDevice"""
        stream = sd.OutputStream(samplerate=sample_rate, channels=channels, dtype=dtype)
        stream.start()
        try:
            for audio_bytes in audio_stream:
                stream.write(audio_bytes.audio_int16_array) # Assuming audio_stream yields audio chunks
    ...
    def validate_and_clean_audio(audio_data: np.ndarray) -> np.ndarray:
        ...
        if has_invalid:
            print(f"⚠️ Audio validation completed, RMS energy: {np.sqrt(np.mean(audio_array**2)):.6f}")
...
class STTEngine:
    ...
    def _initialize_model(self):
        ...
        try:
            if self.config.stt_engine == STTBackend.WHISPER:
                ...
                print(f"🧠 Loading Faster Whisper {self.config.faster_whisper_model_size} model...")
    ...
    def transcribe(self, audio_data: np.ndarray) -> str:
        ...
        try:
            if self.config.stt_engine == STTBackend.WHISPER or self.config.stt_engine == STTBackend.FASTER_WHISPER:
    ...
    def _transcribe_whisper(self, audio_data: np.ndarray) -> str:
        ...
        try:
            ...
            if clean_audio.size == 0 or len(clean_audio) < 1600:  # Less than 0.1 seconds at 16kHz
                return ""
            ...
            if isinstance(result, dict):
                ...
                segments = getattr(result, 'segments', result) if hasattr(result, 'segments') else result
            ...
            if segments:
                for segment in segments:
                    ...
                    text = segment.get('text', '') if isinstance(segment, dict) else getattr(segment, 'text', '') if hasattr(segment, 'text') else segment if isinstance(segment, str) else ''
    ...
    def _synthesize_voice_to_wav(self, text, output_file):
        ...
        if self.config.tts_to_cli:
            ...
            with wave.open(output_file, "wb") as wav_file:
                self.voice.synthesize_wav(text=text, wav_file=wav_file, set_wav_format=True, syn_config=self.config.synthesis_config)
    ...
    def speak(self, text: str) -> bool:
        ...
        if self.config.tts_to_cli: 
            ...
            try:
                ...
                AudioUtils.play_audio_stream(audio_stream, sample_rate=self.voice.config.sample_rate)
                return True
            except Exception as e:
                print(f"❌ TTS error: {e}")
                return False
...
class AIResponseEngine:
    ...
    def __init__(self):
        ...
        self.intents = {
            ...
            "exit": f"See you later! Going back to sleep mode.",
            "help": f"I can respond to simple commands like hello, time, weather, lights, music, and stop."
        }
    ...
    def generate_response(self, user_input: str) -> Tuple[str, bool]:
        ...
        should_continue = not any(exit_word in user_input for exit_word in ["stop", "exit", "quit"])
    ...
    def _match_intent(self, user_input: str) -> str:
        ...
        # Fallback response
        return f"I heard you say '{user_input}'. I'm still learning how to respond to that."
    ...
    def _setup_audio_stream(self):
            ...
            for i in range(device_count):
                ...
                if max_input_channels > 0:
                    ...
                    if 'pulse' in device_name.lower() or 'default' in device_name.lower():
                        device_index = i
                        print(f"🎤 Using audio device {i}: {device_name} (channels: {max_input_channels})")
                        break
                    elif device_index is None:  # Use first available input device as fallback
                        device_index = i
                        print(f"🎤 Fallback audio device {i}: {device_name} (channels: {max_input_channels})")
        ...
        try:
            self.stream = self.audio.open(
                ...
                input_device_index=device_index  # Use detected device or None for default
            )
        ...
        try:
            while self.is_listening:
                ...
                if len(audio_buffer) >= self.config.sample_rate:
                    ...
                    # Force cleanup
                    #print(f"🧹 Cleaning up memory... wake_word_detected: {self.wake_word_detected}")
                    gc.collect()
    ...
    def _process_audio_buffer(self, audio_buffer: List[float]):
        ...
        try:
            ...
            if not audio_buffer or len(audio_buffer) < 1600:
                print("⚠️ Skip processing if validation resulted in empty or very short audio buffer")
                return
            ...
            # if raw_array.size == 0 or np.all(np.isnan(raw_array)) or np.all(np.isinf(raw_array)):
            #     print("⚠️ Skipping completely invalid audio buffer")
            #     return
            ...
            if rms_energy < 1e-5:  # Very low energy threshold
                print("⚠️ Skip processing very low energy audio buffer")
                return
...
def main():
    ...
    config = AgentConfig(
        ...
        language="en",
        whisper_model_size="tiny",     # tiny for speed, base for accuracy
        faster_whisper_model_size="tiny", # medium.en for better accuracy, base for speed
